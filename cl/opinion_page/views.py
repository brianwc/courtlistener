...
async def view_opinion_authorities(
    ...
    # Extract U.S. Code citations from pre-annotated HTML
    statutory_authorities = []
    seen = set()
    for sub_opinion in cluster.sub_opinions.all():
        if not sub_opinion.html_with_citations:
            continue
        # Regex to locate U.S. Code citation spans generated by eyecite
        for tag_match in re.finditer(r'<span[^>]*class="citation us-code-citation"[^>]*>', sub_opinion.html_with_citations):
            tag_text = tag_match.group(0)
            title_match = re.search(r'data-title="([^"]+)"', tag_text)
            section_match = re.search(r'data-section="([^"]+)"', tag_text)
            if title_match and section_match:
                ...
                if key not in seen:
                    seen.add(key)
                    statutory_authorities.append({
                        "title": title,
                        "section": section,
                        "citation_string": f"{title} U.S.C. § {section}",
                    })

    # Sort statutory authorities numerically by title, then section
    def parse_section(sec):
        m = re.match(r'^\d+', sec)
        return int(m.group(0)) if m else 999999

    try:
        statutory_authorities.sort(key=lambda s: (int(s["title"]), parse_section(s["section"])))
    except Exception:
        pass
